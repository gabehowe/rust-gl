...
impl Camera {
    ...
    pub unsafe fn initialize_buffers(&mut self, shaders: &mut Vec<&mut Shader>) {
        for i in &mut *shaders {
            (*i).bind_matrices()
        }
    ...
    pub fn handle_mouse(&mut self, x: c_double, y: c_double) {
        ...
        self.rot.y -= (delta_x as f32) * sensitivity;
        self.rot.x += (delta_y as f32) * sensitivity;
    }
    pub fn translate(&mut self, vector3: Vector3<f32>) {
        self.pos += Basis3::from(Euler::new(Rad(self.rot.x), Rad(self.rot.y), Rad(0.))).rotate_vector(vector3);
    }
}
