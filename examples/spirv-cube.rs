...
fn main() {
    ...
    let spirv = Shader::from_spirv_binary("shaders/target/spirv-unknown-opengl4.5/debug/deps/shaders.spv", vec![]).expect("Couldn't load SPIR-V binary!");
    let shader = engine.data.shader_manager.register(spirv);
